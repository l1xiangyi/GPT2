...
def main():
    ...
    from questions.sample import sample
    paper_iter = iter(paper_dataset)
    with open(os.path.join('submit', 'samples_temperature0-95.txt'), 'w', encoding='utf-8') as f:
        for i in range(5):
            ## Use paper abstracts as the starting text
            start_text = next(paper_iter)['abstract'][:100]
            start_text = codec.encode(start_text).to(device)
    ...
    from questions.sample import sample
    paper_iter = iter(paper_dataset)
    with open(os.path.join('submit', 'samples_temperature0-95_horizon2.txt'), 'w', encoding='utf-8') as f:
        for i in range(1):
            ## Use paper abstracts as the starting text
            start_text = next(paper_iter)['abstract'][:100]
            start_text = codec.encode(start_text).to(device)
